...
class DeviceBackend(ModelBackend):
    ...
    def authenticate(self, request, mobile_device_id=None, mobile_login_token=None, **kwargs):
        ...
        if not mobile_device_qs.exists():
            raise PermissionDenied('MobileDevice with id "{}" not found.'.format(device_uuid))
        ...
        raise PermissionDenied('Provided invalid login_token to MobileDevice with id "{}".'.format(device_uuid))
